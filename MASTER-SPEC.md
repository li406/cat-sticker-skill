...
- Seedream provider: size must be >= 3686400 px (using 2048x2048), response_format=url, reference image as data URI
- flood-fill: pure NumPy BFS, no SciPy dependency

## Phase 4

- Banner is generated by the host Agent built-in image generation (not Seedream), to avoid stretching a main image
- Cover/Icon are scaled from the verified clean generated.png

## Phase 5

- flood-fill edge feathering: GaussianBlur(1.5px) on alpha + near-white pixel removal, eliminates white halo
- Typography presets: meme-yellow, clean-white, cute-soft, bold-contrast
- Multi-line text support via newline
- Privacy scanner: EXIF check (GPS, camera model)

## Phase 6 Scope

- Only Doubao environment testing, no Codex dual-port test
